  ...
  (let [[db actor] (fresh)
        res (exec-op actor "t1"
                  {:op :engagement/intake :subject "eng-1"
                   :patch {:id "eng-1" :operator "Vientiane Manufacturing Co., Ltd."}} operator)]
    (is (= :commit (get-in res [:state :disposition])))
    (is (= "Vientiane Manufacturing Co., Ltd." (:operator (store/engagement db "eng-1"))) "SSoT actually updated")
...
(deftest legislative-approval-mismatch-is-held-and-unoverridable
  (testing "declared :claimed-legislative-approval does not match the independently recomputed Art.49/50 tier -> HARD hold (flagship check)"
    (let [[db actor] (fresh)
      ...
      (is (some #{:legislative-approval-mismatch} (-> (store/ledger db) last :basis)))
...
(deftest enterprise-registration-unverified-is-held-and-unoverridable
  (testing "unverified enterprise registration certificate when required -> HARD hold"
    (let [[db actor] (fresh)
      ...
      (is (= :hold (get-in res [:state :disposition])) "settles immediately, no interrupt")
      (is (not= :interrupted (:status res)))
      (is (some #{:enterprise-registration-unverified} (-> (store/ledger db) last :basis)))
      (is (empty? (store/submit-history db))))))
