...
(defn- exercise [s]
  (store/commit-record! s {:effect :engagement/upsert
                           :value {:id "eng-x" :operator "X Co., Ltd." :jurisdiction "LAO"
                                   :base-fee 100 :monthly-rate 10 :monitoring-months 1
                                   :claimed-fee 110.0
                                   :land-concession-hectares 12000
  ...
  (store/commit-record! s {:effect :assessment/set
                           :path ["eng-x"]
                           :payload {:jurisdiction "LAO" :checklist ["a"] :spec-basis "x"}})
...
(deftest mem-and-datomic-parity
  (let [mem (store/seed-db)
    ...
    (is (= (:land-concession-hectares (:engagement m)) (:land-concession-hectares (:engagement d))))
    (is (= (:claimed-legislative-approval (:engagement m)) (:claimed-legislative-approval (:engagement d))))
