(ns marketentry.marketentryllm
  ...
  the Lao People's Democratic Republic (LAO) public-sector market-entry
  compliance actor.
  ...
  cited), never a committed record or a real investment-license /
  enterprise-registration filing. Every output is censored downstream
  by `marketentry.governor` before anything touches the SSoT, and
...
(defn- propose-submit
  ...
  :actuation/submit-filing` -- real-world investment-license /
  enterprise-registration filing request."
  [db {:keys [subject]}]
  (let [e (store/engagement db subject)]
    {:summary    (str subject " 向け投資ライセンス/企業登録提出提案"
                      (when e (str " (operator=" (:operator e) ")")))
     :rationale  (if e
                   (str "land-concession-hectares=" (:land-concession-hectares e)
                        " relocated-households=" (:relocated-households e)
                        " claimed-legislative-approval=" (:claimed-legislative-approval e)
                        " enterprise-registration-verified?=" (:enterprise-registration-verified? e)
     ...
     :confidence (if (and e
                          (or (not (:requires-enterprise-registration? e))
                              (:enterprise-registration-verified? e)))
                   0.9 0.3)}))
